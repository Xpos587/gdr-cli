...
__version__ = "0.1.0"
from exceptions import AuthError, GDRError
...
def _handle_api_error(e: Exception) -> int | None:
  ...
  from gemini_webapi.exceptions import TemporarilyBlocked, UsageLimitExceeded
...
def main(
  ...
  debug: bool = typer.Option(
    ...
    "--debug",
    "-d",
    help="Enable debug logging from gemini_webapi",
  ),
):
  if not debug:
...
@app.command()
def chat(
  prompt: str | None = typer.Argument(default=None, help="Message to send to Gemini"),
  ...
  continue_chat: str | None = typer.Option(
    None,
    "--continue",
    "-c",
    help="Continue chat by CID, e.g. 1975e4a9e33a362 (omit for last chat)",
  ),
  model: str | None = typer.Option(
    None,
    "--model",
    "-m",
    help="Model name, e.g. gemini-3-pro-advanced (default: auto)",
  ),
):
    ...
    if prompt:
      if metadata:
        response = asyncio.run(
          _chat_mod.continue_chat(prompt, metadata=metadata, profile=profile, model=model)
        )
      else:
...
def chats_show(
  cid: str = typer.Argument(
    help="Chat ID to display (e.g. 1975e4a9e33a362, with or without c_ prefix)"
  ),
  profile: str = typer.Option(
    "default",
    "--profile",
...
def research(
  ...
  timeout: int = typer.Option(
    ...
    "--timeout",
    "-t",
    help="Max research time in minutes",
  ),
  ...
  no_confirm: bool = typer.Option(
    ...
    "--no-confirm",
    "-n",
    help="Skip plan confirmation and start research immediately",
  ),
  ...
  output_dir: Path | None = typer.Option(
    None,
    "--output-dir",
    help="Auto-save report to DIR/{date}-{slug}.md",
  ),
  model: str | None = typer.Option(
    None,
    "--model",
...
def doctor(
  ...
  if not profile_dir.exists():
    ...
    console.print("  Run [bold]gdr login[/bold] first to authenticate.")
    raise typer.Exit(1)
  ...
  if not cookies_file.exists():
    ...
    console.print("  Run [bold]gdr login[/bold] first.")
    raise typer.Exit(1)
  ...
  if missing:
    ...
    console.print("  [green]OK[/green] Required cookies present")
  ...
  if metadata_file.exists():
    ...
    console.print("  [yellow]WARN[/yellow] No metadata file (optional)")

  # 5. Test Gemini connectivity
  console.print("\n  Testing Gemini connectivity...")
  try:
    tier, dr_status = asyncio.run(_test_connectivity(cookies))
    console.print("  [green]OK[/green] Gemini: authenticated and accessible")
...
def login(
  profile: str = typer.Option(
    ...
    "--profile",
    "-p",
    help="Auth profile name (shared with nlm)",
  ),
  ...
  launch: bool = typer.Option(
    ...
    "--launch/--no-launch",
    "-l",
    help="Auto-launch Chrome if CDP not reachable",
  ),
):
  ...
  try:
    result = asyncio.run(_async_login(profile=profile, cdp_url=cdp_url, auto_launch=launch))
  except Exception as e:
    console.print(f"  [red]Error:[/red] {e}")
    raise typer.Exit(1)
...
async def _async_login(profile: str, cdp_url: str, auto_launch: bool) -> dict:
  """Run CDP login in a thread pool (CDP uses sync websocket)."""
  from cdp import login_via_cdp

  loop = asyncio.get_event_loop()
  return await loop.run_in_executor(None, login_via_cdp, profile, cdp_url, auto_launch)
