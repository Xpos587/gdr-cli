...
async def run_deep_research(
  ...
  try:
          ...
          while time.monotonic() < deadline:
            ...
            try:
              ...
              if status_obj:
                ...
                if status_obj.title:
                  status.update(
                    f"[bold blue]Research in progress...[/] [{state_label}] {status_obj.title}"
                  )
                if status_obj.done:
            ...
            remaining = int(deadline - time.monotonic())
            status.update(
              f"[bold blue]Research in progress...[/] ({elapsed}s elapsed, ~{remaining}s remaining)"
            )
    ...
    if response_text and "research requests running right now" in response_text.lower():
      ...
      raise GDRError(
        "Gemini research limit reached (3 concurrent max)",
        hint="Wait for existing research to complete or check Gemini web interface",
      )
