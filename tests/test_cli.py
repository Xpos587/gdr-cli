...
from cli import _test_connectivity, app
...
class TestHelp:
  ...
  def test_debug_flag_enabled(self):
    with patch("gemini_webapi.set_log_level") as mock_set:
      runner.invoke(app, ["--debug", "chat", "--help"])
...
class TestErrorHandling:
  def test_chat_auth_error_shows_hint(self):
    ...
    with patch(
      ...
      side_effect=GDRError("No cookies", hint="Run 'gdr login'"),
    ):
  ...
  def test_research_rate_limit(self):
    ...
    with patch(
      ...
      new_callable=AsyncMock,
      side_effect=UsageLimitExceeded("limited"),
    ):
  ...
  def test_research_generic_error(self):
    with patch(
      "research.run_deep_research", new_callable=AsyncMock, side_effect=RuntimeError("fail")
    ):
      result = runner.invoke(app, ["research", "test"])
    assert result.exit_code == 1
  ...
  def test_research_gdr_error(self):
    ...
    with patch(
      "research.run_deep_research",
      new_callable=AsyncMock,
      side_effect=GDRError("auth fail", hint="login"),
    ):
      result = runner.invoke(app, ["research", "test"])
    assert result.exit_code == 2
  ...
  def test_research_usage_limit(self):
    ...
    with patch(
      "research.run_deep_research", new_callable=AsyncMock, side_effect=UsageLimitExceeded("limit")
    ):
      result = runner.invoke(app, ["research", "test"])
    assert result.exit_code == 3
  ...
  def test_research_ip_blocked(self):
    ...
    with patch(
      "research.run_deep_research",
      new_callable=AsyncMock,
      side_effect=TemporarilyBlocked("blocked"),
    ):
      result = runner.invoke(app, ["research", "test"])
    assert result.exit_code == 3
...
class TestChatCommand:
  ...
  def test_chat_single_message_with_continue(self):
    with patch(
      "chat.continue_chat", new_callable=AsyncMock, return_value="Continued!"
    ) as mock_cont:
...
class TestChatsList:
  ...
  def test_chats_list_gdr_error(self):
    ...
    with patch(
      "chat.list_recent_chats", new_callable=AsyncMock, side_effect=GDRError("fail", hint="login")
    ):
      result = runner.invoke(app, ["chats", "list"])
    assert result.exit_code == 2
...
class TestChatsShow:
  ...
  def test_chats_show_cid_without_prefix(self):
    ...
    with patch(
      "chat.read_chat_history", new_callable=AsyncMock, return_value=mock_history
    ) as mock_read:
...
class TestResearchCommand:
  ...
  def test_research_passes_options(self):
    ...
    with patch(
      "research.run_deep_research", new_callable=AsyncMock, return_value=mock_result
    ) as mock_run:
...
class TestDoctorCommand:
  def test_doctor_missing_profile_dir(self, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
      "config.get_cookies_file", lambda n: tmp_path / "nonexistent" / "cookies.json"
    )
    monkeypatch.setattr(
      "config.get_metadata_file", lambda n: tmp_path / "nonexistent" / "metadata.json"
    )
  ...
  def test_doctor_full_success(self, tmp_path, monkeypatch):
    ...
    mock_client.inspect_account_status = AsyncMock(
      return_value={
  ...
  def test_doctor_missing_optional_cookies(self, tmp_path, monkeypatch):
    ...
    mock_client.init = AsyncMock()
    mock_client.inspect_account_status = AsyncMock(
      return_value={"summary": {}, "rpc": {"bootstrap": {"ok": False}}}
    )
    mock_client.close = AsyncMock()
