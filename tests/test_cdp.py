...
class TestLoginViaCdp:
  ...
  @patch(
    "cdp.get_page_html",
    return_value='"oPEP7c":"user@gmail.com" "SNlM0e":"csrf123" "FdrFJe":"456" "cfb2h":"label1"',
  )
  ...
  def test_full_login_flow(
    self, mock_check, mock_find_page, mock_url, mock_logged, mock_html, mock_cookies
  ):
  ...
  @patch("cdp.find_or_create_page", return_value={"webSocketDebuggerUrl": "ws://127.0.0.1:9222/ws"})
  @patch("cdp.check_cdp", return_value=True)
  def test_raises_on_login_timeout(
    self,
    mock_check,
    mock_find_page,
    mock_logged,
  ...
  @patch("cdp.find_or_create_page", return_value={"webSocketDebuggerUrl": "ws://127.0.0.1:9222/ws"})
  @patch("cdp.check_cdp", return_value=True)
  def test_raises_on_no_cookies(
    self, mock_check, mock_find_page, mock_url, mock_logged, mock_cookies
  ):
  ...
  @patch("cdp.find_or_create_page", return_value={"webSocketDebuggerUrl": "ws://127.0.0.1:9222/ws"})
  @patch("cdp.check_cdp", return_value=True)
  def test_handles_missing_email(
    self, mock_check, mock_find_page, mock_url, mock_logged, mock_html, mock_cookies
  ):
    with patch("auth.AuthManager") as mock_auth_cls:
      mock_mgr = MagicMock()
  ...
  @patch("cdp.find_or_create_page", return_value={"webSocketDebuggerUrl": "ws://127.0.0.1:9222/ws"})
  @patch("cdp.check_cdp", return_value=True)
  def test_navigates_to_gemini_if_needed(
    self,
    mock_check,
    mock_find_page,
    mock_logged,
  ...
  @patch("cdp.find_or_create_page", return_value={"webSocketDebuggerUrl": "ws://127.0.0.1:9222/ws"})
  @patch("cdp.check_cdp", return_value=True)
  def test_waits_for_consent_redirect(
    self,
    mock_check,
    mock_find_page,
    mock_url,
